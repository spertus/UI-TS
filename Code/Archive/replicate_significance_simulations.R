...
n_grid <- round(seq(5, 500, length.out = 100))
...
#is this also the case when the sampling is unstratified?
sam_250 <- c(rep(mu, 249), 0)
sam_270 <- c(rep(mu, 269), 0)
SE_250 <- sd(sam_250) / sqrt(250)
SE_270 <- sd(sam_270) / sqrt(270)
T_250 <- (mean(sam_250) - 0.5) / SE_250
T_270 <- (mean(sam_270) - 0.5) / SE_270
P_250 <- 1 - pt(T_250, df = 250 - 1)
P_270 <- 1 - pt(T_270, df = 270 - 1)
