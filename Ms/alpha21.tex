...
\institute{University of California, Berkeley \\ \today}
...
\abstract{
     ...
     BRAVO \cite{lindemanEtal12}, based on Wald's sequential probability ratio test for the Bernoulli distribution, 
     is the most widely tried method for RLAs.
     It has limitations.
     ...
     be used with stratified sampling;
     (iii)~works not only for ballot-polling but also for ballot-level comparison, batch-polling, and batch-level comparison audits,
     sampling with or without replacement, uniformly or with weights proportional to a measure of size;
     (iv)~works for all social choice functions covered by SHANGRLA \cite{stark20}, 
     including approval voting, Borda count, and all scoring rules;
     and (v)~in situations where both ALPHA and BRAVO apply, requires smaller samples than BRAVO
     when the reported vote shares are wrong but the outcome is correct---five orders of 
     magnitude in some examples.
     ALPHA includes the family of betting martingale tests in RiLACS \cite{waudby-smithEtal21}, with a different betting
     strategy parametrized as an estimator of the population mean and the flexibility to accommodate sampling
...
Like the Kaplan-Wald \cite{stark09b,stark20}, Kaplan-Kolmogorov \cite{stark20}, Kaplan martingale \cite{starkEvans19,stark20}, 
RiLACS \cite{waudby-smithEtal21}, and BRAVO \cite{lindemanEtal12}, it is based on Ville's inequality for nonnegative martingales \cite{ville39}.
...
\vspace{.4in}
\noindent {\bf Acknowledgements.} I am grateful to Andrew Appel, Aaditya Ramdas, Jacob Spertus, Damjan Vukcevic, and Ian Waudby-Smith for comments on earlier drafts.
